        ...
        loading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {cases.map((caseItem) => (
                <Link href={`/cases/${caseItem.case_number}`} key={caseItem.case_number}>
                  <Card className="bg-[#ECEBF2] p-4 space-y-2 rounded-xl">
                    <CardContent className="space-y-2">
                      <p className="font-bold text-sm text-gray-700">{caseItem.case_number}</p>
                      <p className="text-xl font-bold">{caseItem.case_name}</p>
                      <div className="flex justify-between text-sm text-red-700 font-semibold">
                        <p>👥 {caseItem.num_victims}</p>
                        <p>฿ - {caseItem.estimated_financial_damage}</p>
                        <p>📅 {caseItem.timestamp}</p>
                      </div>
                      <p className="text-sm text-gray-800 line-clamp-4">
                        {caseItem.description}
                      </p>
                      <div className="flex gap-1 items-center">
                        <p className="text-yellow-700 font-semibold">{(caseItem.priority_score / 20).toFixed(1)}</p>
                        <div className="text-yellow-500 text-lg">
                          {"⭐".repeat(getStarRating(caseItem.priority_score))}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <div className="flex justify-center items-center gap-2 pt-6">
              {pagination && Array.from({ length: pagination.total_pages }, (_, i) => i + 1).map((page) => (
                <Button
                  key={page}
                  size="icon"
                  onClick={() => fetchCases(page, filters, searchTerm)}
                  className={`rounded-full w-9 h-9 ${page === pagination.page ? "bg-blue-900 text-white" : "bg-white text-black"}`}
                >
                  {page}
                </Button>
              ))}
